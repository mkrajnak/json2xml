...
touch $LOG
ARCHIVE=`basename $1`
NAME=`echo $ARCHIVE | cut -d . -f 1 | egrep "^x[a-z]{5}[0-9][0-9a-z]-(CHA|CST|CSV|DKA|JMP|JSN|MKA|SYN|XTD|XQR)$"`
TASK=`echo $ARCHIVE | cut -d . -f 1 | sed 's/^x[a-z]\{5\}[0-9][0-9a-z]-\(CHA\|CST\|CSV\|DKA\|JMP\|JSN\|MKA\|SYN\|XTD\|XQR\)$/\1/'`
...
if [[ -f rozsireni ]]; then
  ...
  RETCODE=$?
  if [[ $RETCODE = "0" ]]; then
    UNKNOWN=`cat rozsireni | grep -v -E -e "^(TER|IND|FNC|COM|PAD|VLC|WCH|RUL|STR|WSA|FUN|PAR|JPD|FLA|JER|WHT|RLO|MST|MWS|HTM|NQS|ORD|LOG|VAL)$" | wc -l`
    if [[ $UNKNOWN = "0" ]]; then
      echo "OK"
    else
